...
export function IntegrationHub({ applications, career, language, gmailConnected }: { applications: Application[]; career: CareerData; language: "tr" | "de"; gmailConnected: boolean }) {
  ...
  const drive = async () => {
    ...
    try { const form = new FormData(); form.set("applicationId", String(selected.id)); const response = await fetch("/api/integrations/drive", { method: "POST", body: form }); const data = await readResponse(response); setMessage(data.folderUrl ? (de ? "Drive-Ordner bereit." : "Drive klasörü hazır.") : (de ? "Drive gespeichert." : "Drive kaydedildi.")); if (data.folderUrl) window.open(data.folderUrl, "_blank", "noopener,noreferrer"); } catch (error) { showError(error); } finally { setBusy(""); }
  };
  const draft = async () => {
    if (!selected) return;
