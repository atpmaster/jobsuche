...
export function IntegrationHub({ applications, career, language, gmailConnected }: { applications: Application[]; career: CareerData; language: "tr" | "de"; gmailConnected: boolean }) {
  ...
  const copy = de ? { title: "Integrationen", hint: "Bewerbung mit Kalender, Drive und Gmail verknüpfen.", choose: "Bewerbung", calendar: "Google-Kalendereintrag hinzufügen", drive: "Drive-Ordner öffnen", draft: "Gmail-Entwurf erstellen", job: "Stellen-URL erfassen", read: "Stelle auslesen", save: "Bewerbung speichern", connected: "Gmail-Verbindung ist bereit", reconnect: "Gmail erneut verbinden", interview: "Kein Gesprächstermin für diese Bewerbung gefunden." } : { title: "Entegrasyonlar", hint: "Başvuru kaydını Takvim, Drive ve Gmail ile eşleştir.", choose: "Başvuru", calendar: "Google Takvim'e ekle", drive: "Drive klasörünü aç", draft: "Gmail taslağı oluştur", job: "İlan bağlantısını al", read: "İlanı oku", save: "Başvuruyu kaydet", connected: "Gmail bağlantısı hazır", reconnect: "Gmail'i yeniden bağla", interview: "Bu başvuru için mülakat kaydı bulunamadı." };
  return <section className="integration-hub career-tools" aria-label={copy.title}><details open><summary>{copy.title}</summary><p>{copy.hint}</p>{!gmailConnected && <p className="integration-warning">{de ? "Google-Integrationen funktionieren erst nach der Gmail-Verbindung." : "Google entegrasyonları için önce Gmail hesabını bağla."} <a href="/api/gmail/connect">{copy.reconnect}</a></p>}{gmailConnected && <p className="integration-scope-note">{de ? "Für neue Google-Berechtigungen kann eine einmalige erneute Verbindung erforderlich sein." : "İlk kullanımda yeni Google izinlerini etkinleştirmek için Gmail hesabını bir kez yeniden bağla."} <a href="/api/gmail/connect">{copy.reconnect}</a></p>}<label>{copy.choose}<select value={selected ? String(selected.id) : ""} onChange={(event) => selectApplication(event.target.value)}>{active.map((application) => <option key={application.id} value={application.id}>{application.company} — {application.role}</option>)}</select></label>{selected && <div className="integration-grid"><div className="integration-card"><strong>{de ? "Google Kalender" : "Google Takvim"}</strong><small>{interviewIds.has(selected.id) ? (de ? "Gesprächstermin ist hinterlegt" : "Mülakat tarihi hazır") : copy.interview}</small>{selected.calendarEventUrl ? <a href={selected.calendarEventUrl} target="_blank" rel="noreferrer">{de ? "Termin öffnen ↗" : "Etkinliği aç ↗"}</a> : <button type="button" onClick={calendar} disabled={busy === "calendar" || !interviewIds.has(selected.id)}>{busy === "calendar" ? "…" : copy.calendar}</button>}</div><div className="integration-card"><strong>Google Drive</strong><small>{selected.driveFolderUrl ? (de ? "Ordner verbunden" : "Klasör bağlı") : (de ? "Bewerbungsordner" : "Başvuru klasörü")}</small><input type="file" accept="application/pdf,.pdf" onChange={(event) => setDriveFile(event.target.files?.[0] || null)} />{selected.driveFolderUrl ? <a href={selected.driveFolderUrl} target="_blank" rel="noreferrer">{de ? "Ordner öffnen ↗" : "Klasörü aç ↗"}</a> : null}<button type="button" onClick={drive} disabled={busy === "drive"}>{busy === "drive" ? "…" : driveFile ? (de ? "PDF in Drive speichern" : "PDF'yi Drive'a kaydet") : copy.drive}</button></div><div className="integration-card integration-draft"><strong>{de ? "Gmail-Entwurf" : "Gmail taslağı"}</strong><small>{copy.connected}</small><input value={subject} onChange={(event) => setSubject(event.target.value)} placeholder={de ? "Betreff" : "Konu"} /><textarea rows={4} value={body} onChange={(event) => setBody(event.target.value)} placeholder={de ? "Entwurf" : "Taslak"} /><button type="button" onClick={draft} disabled={busy === "draft" || !subject || !body}>{busy === "draft" ? "…" : copy.draft}</button></div><div className="integration-card"><strong>{de ? "Gmail-Label" : "Gmail etiketi"}</strong><small>{de ? "Status in den Gmail-Thread schreiben" : "Durumu Gmail ileti dizisine yaz"}</small><button type="button" onClick={label} disabled={busy === "label" || !selected.gmailThreadId}>{busy === "label" ? "…" : (de ? "Label aktualisieren" : "Etiketi güncelle")}</button></div></div>}{message && <p role="status">{message}</p>}</details><details><summary>{copy.job}</summary><p>{de ? "Links von Arbeitsagentur, Indeed, LinkedIn, StepStone und XING in einen Bewerbungseintrag umwandeln." : "Arbeitsagentur, Indeed, LinkedIn, StepStone ve XING bağlantılarını başvuru taslağına dönüştür."}</p><div className="integration-url-row"><input type="url" value={jobUrl} onChange={(event) => setJobUrl(event.target.value)} placeholder="https://…" /><button type="button" onClick={analyzeJob} disabled={busy === "job" || !jobUrl}>{busy === "job" ? "…" : copy.read}</button></div>{jobDraft && <div className="job-capture-result"><label>{de ? "Arbeitgeber" : "Şirket / kurum"}<input value={jobDraft.company} onChange={(event) => setJobDraft({ ...jobDraft, company: event.target.value })} /></label><label>{de ? "Position" : "Pozisyon"}<input value={jobDraft.role} onChange={(event) => setJobDraft({ ...jobDraft, role: event.target.value })} /></label><small>{jobDraft.source} · {jobDraft.url}</small><button type="button" onClick={saveJob} disabled={busy === "job-save"}>{busy === "job-save" ? "…" : copy.save}</button></div>}</details></section>;
}
