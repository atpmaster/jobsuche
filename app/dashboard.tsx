...
export function Dashboard({ applications: allApplications, tasks, today, career, gmailConnected }: DashboardProps) {
  ...
  const pdfApplications = reportApplications.filter((item) => pdfScope === "all" || (pdfScope === "interview" ? isInterviewInvitation(item, interviewApplicationIds) : !isInterviewInvitation(item, interviewApplicationIds)));
  ...
  const printReport = async () => {
    ...
    try {
      ...
      const rows = [...pdfApplications].sort((a, b) => {
        ...
        next: [isInterviewInvitation(item, interviewApplicationIds) ? interviewScheduleFor(item) : "", item.nextActionDate ? new Intl.DateTimeFormat(locale).format(new Date(`${item.nextActionDate}T12:00:00`)) : "", localizedContent(item.nextAction, language) || t.noNextAction].filter(Boolean).join("\n"),
      }));
      const period=[dateFrom||"…",dateTo||"…"].join(" – ");
      const scopeLabel = pdfScope === "interview" ? t.pdfInterviews : pdfScope === "withoutInterview" ? t.pdfWithoutInterviews : t.pdfAll;
      ...
      const fileScope = pdfScope === "interview" ? (language === "de" ? "Nur-Vorstellungsgespraeche" : "Sadece-Mulakatlar") : pdfScope === "withoutInterview" ? (language === "de" ? "Ohne-Vorstellungsgespraeche" : "Mulakat-Haric") : "";
      ...
      <div className="screen-ui">
        ...
        <section className="workspace">
          ...
          <section className="record-controls" aria-label={language==="de"?"Suche und Bericht":"Arama ve rapor"}>
            ...
            <label>{t.pdfScope}<select value={pdfScope} onChange={e=>setPdfScope(e.target.value as PdfScope)}><option value="all">{t.pdfAll}</option><option value="interview">{t.pdfInterviews}</option><option value="withoutInterview">{t.pdfWithoutInterviews}</option></select></label>
