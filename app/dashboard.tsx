  ...
  const printReport = async () => {
    ...
    try {
      ...
      const rows = [...pdfApplications].sort((a, b) => {
        ...
        source: localizedSource(item.source, language) || t.noSource, status: statuses[item.status] || item.status, statusKey: item.status,
      ...
      <div className="screen-ui">
        ...
        <section className="workspace">
          ...
          <section className="record-controls" aria-label={language==="de"?"Suche und Bericht":"Arama ve rapor"}>
            ...
            <div className="control-actions"><button type="button" onClick={()=>{setQuery("");setFilterStatus("");setFilterTrack("");setDateFrom("");setDateTo("");setSummaryFilter("");setPdfScope("all");}}>{language==="de"?"Filter zurücksetzen":"Filtreleri temizle"}</button><button type="button" onClick={exportBackup}>{language==="de"?"Datensicherung (JSON)":"Verileri yedekle (JSON)"}</button></div>
          ...
          <div className="file-layout">
            <section className={`file-list ${interviewCount ? "has-interview-group" : ""}`} id="basvurular" aria-label={language === "de" ? "Bewerbungsverfolgung" : "Başvuru takibi"}>
              ...
              {filtered.map((item, index) => {
                ...
                const hasRejection = item.status === "rejected";
