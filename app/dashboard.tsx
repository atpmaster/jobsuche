...
import { localizedContent, localizedLabel, localizedSource, responseClassificationLabels, statusLabels, type Language } from "./localization";
import { formatInterviewDetails, getInterviewDetails, isConfirmedInterview } from "./interview-utils";
import { type ResponseClassification } from "./response-classification";
...
export type Application = {
  ...
  nextAction: string | null; nextActionDate: string | null; feedback: string | null; responseClassification: ResponseClassification; gmailMessageId?: string | null; gmailThreadId?: string | null; steps: Step[]; updates: Update[];
...
function isInterviewInvitation(application: Application, interviewApplicationIds: ReadonlySet<number>) {
  if (application.responseClassification === "interview") return true;
  if (interviewApplicationIds.has(application.id)) return true;
  return isConfirmedInterview(application.updates);
}

function responseLabel(application: Application, language: Language) {
  return responseClassificationLabels[language][application.responseClassification] || responseClassificationLabels[language].none;
}

function localizedNextAction(application: Application, language: Language) {
  if (application.responseClassification === "interview") {
    return localizedContent("Mülakat teyit edildi; görüşmeye hazırlan", language);
  }
  if (application.responseClassification === "rejected") {
    return localizedContent("Başka işlem gerekmiyor", language);
  }
  return localizedContent(application.nextAction, language);
}
...
function ApplicationAuditEditor({ item, language }: { item: Application; language: Language }) {
  ...
  return <details className="audit-editor"><summary>{de ? "Berichtsfelder prüfen und korrigieren" : "Rapor alanlarını kontrol et ve düzelt"}: {item.company} — {item.role}</summary><form action={updateApplicationRecord} className="audit-form">
    ...
    <div className="form-row"><label>{de ? "Bereich" : "Alan"}<select name="track" defaultValue={item.track}><option value="teaching">{de ? "Bildung" : "Eğitim"}</option><option value="cyber">{de ? "Cybersecurity" : "Siber güvenlik"}</option><option value="other">{de ? "Sonstige" : "Alternatif"}</option></select></label><label>{de ? "Status" : "Durum"}<select name="status" defaultValue={item.status}>{statusOrder.map(status => <option key={status} value={status}>{statusLabels[language][status]}</option>)}</select></label><label>{de ? "Klassifizierung der Rückmeldung" : "Gelen cevabın sınıflandırması"}<select name="responseClassification" defaultValue={item.responseClassification}>{responseClassificationOrder.map(value => <option key={value} value={value}>{responseClassificationLabels[language][value]}</option>)}</select></label></div>
...
export function Dashboard({ applications: allApplications, tasks, today, career, gmailConnected }: DashboardProps) {
  ...
  const interviewApplicationIds = new Set(career.interviews.map((event) => event.applicationId));
  ...
  const [responseFilter, setResponseFilter] = useState<ResponseClassification | "">("");
  ...
  const reportFooter = language === "de"
    ? "Nach den im System erfassten Angaben erstellt. Erstellt von Ahmet Tepe mit einem KI-gestützten Automatisierungssystem."
    : "Sisteme girilen bilgilere göre hazırlanmıştır. Ahmet Tepe tarafından Yapay Zekâ destekli otomasyon sistemi ile oluşturulmuştur.";
  ...
  const rejectedApplications = applications.filter((item) => item.responseClassification === "rejected");
  ...
  const filtered = applications.filter(item => (!query || normalize([item.company,item.role,item.location].join(" ")).includes(normalize(query))) && (!filterStatus || item.status === filterStatus) && (!responseFilter || item.responseClassification === responseFilter) && (!filterTrack || item.track === filterTrack) && (!dateFrom || !!item.appliedOn && item.appliedOn >= dateFrom) && (!dateTo || !!item.appliedOn && item.appliedOn <= dateTo) && (!summaryFilter || (summaryFilter === "open" && openStatuses.has(item.status)) || (summaryFilter === "waiting" && item.status === "waiting") || (summaryFilter === "advanced" && item.status === "received") || (summaryFilter === "due" && followUps.some((followUp) => followUp.id === item.id)) || (summaryFilter === "interview" && item.responseClassification === "interview") || (summaryFilter === "rejected" && item.responseClassification === "rejected")));
  const reportApplications = filtered;
  const pdfApplications = reportApplications.filter((item) => {
    if (pdfScope === "all") return true;
    if (pdfScope === "interview") return isInterviewInvitation(item, interviewApplicationIds);
    if (pdfScope === "rejected") return item.responseClassification === "rejected";
    return !isInterviewInvitation(item, interviewApplicationIds);
  });
  ...
  const interviewEventFor = (applicationId: number) => career.interviews.find((event) => event.applicationId === applicationId);
  const interviewScheduleFor = (application: Application) => formatInterviewDetails(getInterviewDetails(application.updates, interviewEventFor(application.id)?.startsAt), language);
  const focusSummary = (nextFilter: string) => {
    setSummaryFilter(nextFilter);
    setQuery(""); setFilterStatus(""); setResponseFilter(""); setFilterTrack(""); setDateFrom(""); setDateTo("");
  ...
  const printReport = async () => {
    ...
    try {
      ...
      const rows = [...pdfApplications].sort((a, b) => {
        ...
        company: localizedContent(item.company, language) || item.company, role: localizedContent(item.role, language) || item.role, location: localizedContent(item.location, language) || item.location || "",
        date: item.appliedOn ? new Intl.DateTimeFormat(locale).format(new Date(`${item.appliedOn}T12:00:00`)) : t.noDate,
        source: localizedSource(item.source, language) || t.noSource, status: statuses[item.status] || item.status, statusKey: item.status, classification: responseLabel(item, language), classificationKey: item.responseClassification,
      next: [isInterviewInvitation(item, interviewApplicationIds) ? interviewScheduleFor(item) : "", item.nextActionDate ? new Intl.DateTimeFormat(locale).format(new Date(`${item.nextActionDate}T12:00:00`)) : "", localizedNextAction(item, language) || t.noNextAction].filter(Boolean).join("\n"),
      }));
      const period=[dateFrom||"…",dateTo||"…"].join(" – ");
      const scopeLabel = pdfScope === "interview" ? t.pdfInterviews : pdfScope === "rejected" ? t.pdfRejected : pdfScope === "withoutInterview" ? t.pdfWithoutInterviews : t.pdfAll;
      const reportScope=[language==="de"?"Zeitraum: ":"Dönem: ",period,filterStatus?statuses[filterStatus]:t.all,`${t.pdfScope}:`,scopeLabel].join(" ");
      ...
      const fileScope = pdfScope === "interview" ? (language === "de" ? "Nur-Vorstellungsgespraeche" : "Sadece-Mulakatlar") : pdfScope === "rejected" ? (language === "de" ? "Nur-Absagen" : "Sadece-Retler") : pdfScope === "withoutInterview" ? (language === "de" ? "Ohne-Vorstellungsgespraeche" : "Mulakat-Haric") : "";
      const fileName = language === "de" ? `Ahmet-Tepe-Bewerbungsnachweis${fileScope ? `-${fileScope}` : ""}-${today}.pdf` : `Ahmet-Tepe-Basvuru-Takip${fileScope ? `-${fileScope}` : ""}-${today}.pdf`;
      ...
      const emptyReport = error instanceof Error && error.message === "EMPTY_REPORT";
      setPdfError(emptyReport
        ? (language === "de" ? "Für die gewählten Filter sind keine Bewerbungen vorhanden." : "Seçilen filtrelerde PDF'ye alınacak başvuru yok.")
        : (language === "de" ? "PDF konnte nicht erstellt werden. Bitte erneut versuchen." : "PDF oluşturulamadı. Lütfen tekrar deneyin."));
  ...
  const openApplication = (id: number) => {
    ...
    setQuery(""); setFilterStatus(""); setResponseFilter(""); setFilterTrack(""); setDateFrom(""); setDateTo(""); setSummaryFilter("");
    ...
    <main className="file-manager" data-language={language}>
      <div className="screen-ui">
        ...
        <section className="workspace">
          ...
          <div className="file-toolbar" id="takip"><div className="toolbar-title"><span className="folder-tab">A</span><strong>{language === "de" ? "Alle Bewerbungen" : "Tüm başvurular"}</strong><b>{applications.length}</b></div><div className="toolbar-stats"><button type="button" aria-pressed={summaryFilter === "open"} className={`toolbar-stat-link ${summaryFilter === "open" ? "is-selected" : ""}`} onClick={() => focusSummary("open")}><strong>{active.length}</strong> {t.open}</button><button type="button" aria-pressed={summaryFilter === "waiting"} className={`toolbar-stat-link ${summaryFilter === "waiting" ? "is-selected" : ""}`} onClick={() => focusSummary("waiting")}><strong>{awaiting.length}</strong> {t.awaiting}</button><button type="button" aria-pressed={summaryFilter === "advanced"} className={`toolbar-stat-link ${summaryFilter === "advanced" ? "is-selected" : ""}`} onClick={() => focusSummary("advanced")}><strong>{inMotion.length}</strong> {t.advanced}</button><button type="button" aria-pressed={summaryFilter === "due"} className={`toolbar-stat-link ${summaryFilter === "due" ? "is-selected" : ""} ${followUps.length ? "is-alert" : ""}`} onClick={() => focusSummary("due")}><strong>{followUps.length}</strong> {t.due}</button><button type="button" aria-pressed={summaryFilter === "interview"} className={`toolbar-stat-link outcome-stat interview-stat ${summaryFilter === "interview" ? "is-selected" : ""}`} onClick={() => focusSummary("interview")}><strong>{interviewCount}</strong> {t.interviewInvitation}</button><button type="button" aria-pressed={summaryFilter === "rejected"} className={`toolbar-stat-link outcome-stat rejection-stat ${summaryFilter === "rejected" ? "is-selected" : ""}`} onClick={() => focusSummary("rejected")}><strong>{rejectedApplications.length}</strong> {t.rejection}</button><button type="button" aria-pressed={false} className="toolbar-stat-link" onClick={() => focusSummary("")}><strong>%{avgScore}</strong> {t.averageMatch}</button></div><span className="toolbar-date">{todayLabel}</span>{dismissed.size > 0 && <button className="restore-alerts" type="button" onClick={restoreAll}>{t.restoreDismissed}</button>}</div>
          ...
          <section className="record-controls" aria-label={language==="de"?"Suche und Bericht":"Arama ve rapor"}>
            ...
            <label>{t.responseClassification}<select value={responseFilter} onChange={e=>{setSummaryFilter("");setResponseFilter(e.target.value as ResponseClassification | "")}}><option value="">{t.all}</option>{responseClassificationOrder.filter(value => value !== "none").map(value=><option key={value} value={value}>{responseClassificationLabels[language][value]}</option>)}</select></label>
            ...
            <label>{t.pdfScope}<select value={pdfScope} onChange={e=>setPdfScope(e.target.value as PdfScope)}><option value="all">{t.pdfAll}</option><option value="interview">{t.pdfInterviews}</option><option value="rejected">{t.pdfRejected}</option><option value="withoutInterview">{t.pdfWithoutInterviews}</option></select></label>
            <div className="control-actions"><button type="button" onClick={()=>{setQuery("");setFilterStatus("");setResponseFilter("");setFilterTrack("");setDateFrom("");setDateTo("");setSummaryFilter("");setPdfScope("all");}}>{language==="de"?"Filter zurücksetzen":"Filtreleri temizle"}</button><button type="button" onClick={exportBackup}>{language==="de"?"Datensicherung (JSON)":"Verileri yedekle (JSON)"}</button></div>
      <p role="status">{invalidRange?(language==="de"?"Datumsbereich ungültig.":"Tarih aralığı geçersiz."):`${filtered.length} / ${applications.length}`}</p>
          ...
          <IntegrationHub applications={applications} career={career} language={language} gmailConnected={gmailConnected} />
          ...
          <div className="file-layout">
            ...
            <aside className="inspector-column">
              {interviewCount > 0 && <section className="interview-panel" aria-labelledby="interview-panel-heading"><div className="interview-panel-heading"><div><p className="eyebrow"><span className="eyebrow-line" /> {t.interviewInvitation}</p><h2 id="interview-panel-heading">{t.interviewGroupTitle}</h2></div><span className="interview-panel-count">{interviewCount}</span></div><p className="interview-panel-hint">{t.interviewGroupHint}</p><div className="interview-panel-list">{interviewApplications.map((item) => <a className="interview-panel-item" href={`#app-${item.id}`} onClick={() => openApplication(item.id)} key={item.id}><span className="interview-panel-star" aria-hidden="true">★</span><span><strong>{item.role}</strong><small>{item.company} · {item.location || t.noLocation}</small></span><span aria-hidden="true">↗</span></a>)}</div></section>}
              ...
              <section className="print-report" aria-hidden="true"><header className="print-header"><div><p className="print-kicker">{t.pdfTitle}</p><h1>{t.pdfSubtitle}</h1></div><div className="print-meta"><span>{t.generated}</span><strong>{formatDate(today, language, true)}</strong><small>{language === "de" ? "Sprache: Deutsch" : "Dil: Türkçe"}</small></div></header><div className="print-summary"><span><strong>{pdfApplications.length}</strong> {t.applications.toLowerCase()}</span><span><strong>{pdfApplications.filter(item => openStatuses.has(item.status)).length}</strong> {t.open}</span><span><strong>{pdfApplications.filter(item => item.status === "waiting").length}</strong> {t.awaiting}</span><span><strong>{pdfApplications.filter(item => isInterviewInvitation(item, interviewApplicationIds)).length}</strong> {t.interviewInvitation}</span><span><strong>{pdfApplications.filter(item => item.responseClassification === "rejected").length}</strong> {t.rejection}</span><span><strong>%{pdfApplications.length ? Math.round(pdfApplications.reduce((sum, item) => sum + item.score, 0) / pdfApplications.length) : 0}</strong> {t.averageMatch}</span></div><table className="print-table"><thead><tr><th>{t.number}</th><th>{t.employer}</th><th>{t.reportSource}</th><th>{t.reportStatus}</th><th>{t.responseClassification}</th><th>{t.applicationDate}</th><th>{t.reportNext}</th></tr></thead><tbody>{pdfApplications.map((item, index) => <tr key={item.id}><td>{String(index + 1).padStart(2, "0")}</td><td><strong>{localizedContent(item.company, language) || item.company}</strong><br /><span>{localizedContent(item.role, language) || item.role}</span><br /><small>{localizedContent(item.location, language) || item.location || t.noLocation}</small></td><td>{localizedSource(item.source, language) || t.noSource}</td><td><span className={`print-status ${item.status}`}>{statuses[item.status] || item.status}</span></td><td><span className={`print-response response-${item.responseClassification}`}>{responseLabel(item, language)}</span></td><td>{item.appliedOn ? formatDate(item.appliedOn, language, true) : t.noDate}</td><td>{[isInterviewInvitation(item, interviewApplicationIds) ? interviewScheduleFor(item) : "", item.nextActionDate ? formatDate(item.nextActionDate, language, true) : "", localizedNextAction(item, language) || t.noNextAction].filter(Boolean).join(" - ")}</td></tr>)}</tbody></table><section className="print-details"><h2>{language === "de" ? "Hinweise zu den Bewerbungen" : "Başvurular için notlar"}</h2>{pdfApplications.map((item) => <article key={item.id}><h3>{localizedContent(item.company, language) || item.company} - {localizedContent(item.role, language) || item.role}</h3><p><strong>{t.reportContact}:</strong> {item.contactName || t.noContact}{item.contactEmail ? ` · ${item.contactEmail}` : ""}</p><p><strong>{t.notes}:</strong> {localizedContent(item.feedback, language) || localizedContent(item.notes, language) || t.noNote}</p></article>)}</section><footer className="print-footer">{reportFooter}</footer></section>
    </main>
  );
}
